...
toolframe = [151.51,0.00,80.01,0.00,90.00,180.00];
uframe = [1700.00,0.00,1200.00,0.00,0.00,180.00];
...
savepathfile = fopen('path.txt', 'w');
for i = 1:size(pathdata, 1)
    fprintf(savepathfile, '%f,%f,%f,%f,%f,%f\n', pathdata(i, 1),pathdata(i, 2),pathdata(i, 3),pathdata(i, 4),pathdata(i, 5),pathdata(i, 6));
end
fclose(savepathfile);
...
[subSegmentUPara, subSegmentFeedPeakSmall, subSegmentFeedPeakBig] = NurbsScanning(bsplinepath);    % 速度极值曲线扫描

arcLengthVector = NurbsSubSegArcLenCal(subSegmentUPara, bsplinepath);    % 计算各段弧长
...
[interpCor, sVelProfilePlan] = NurbsInterpProcess(subSegmentFeedPeakSmall, subSegmentFeedPeakBig, subSegmentAccAccTime, subSegmentDecDecTime, subSegmentContVTime, subSegmentFinishTime, bsplinepath);
...
% 
velj = joint(2:end, :) - joint(1:end - 1, :) / 0.002;
figure;
...
accj = velj(2:end, :) - velj(1:end - 1, :)/ 0.002;
figure;
...
% 
h = figure;
for i = 1:200:size(joint, 1)
    drawrobotpos(joint(i, :), h);
    hold on;
%     plot3(bpath(:, 1), bpath(:, 2), bpath(:, 3) ,'g', 'linewidth', 2);
end

wrfile = fopen('interp.txt', 'w');
for i = 1:size(interpCor, 1)
    fprintf(wrfile, '%f,%f,%f,%f,%f,%f\n', interpCor(i, 1),interpCor(i, 2),interpCor(i, 3),interpCor(i, 4),interpCor(i, 5),interpCor(i, 6));
end
fclose(wrfile);

wrfile = fopen('interpjoint.txt', 'w');
for i = 1:size(interpCor, 1)
    fprintf(wrfile, '%f,%f,%f,%f,%f,%f\n', joint(i, 1),joint(i, 2),joint(i, 3),joint(i, 4),joint(i, 5),joint(i, 6));
end
fclose(wrfile);